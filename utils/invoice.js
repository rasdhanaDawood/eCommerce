...
function generateCustomerInformation(doc, orderData) {
    ...
    doc
        ...
        .text(`${orderData.user.firstName} ${orderData.user.lastName} `, 150, customerInformationTop)
        .moveDown()
        .text(`Shipping Address:`, 50, customerInformationTop + 15)
        .text(`${orderData.address.address}`, 150, customerInformationTop + 15)
        .text(`${orderData.address.city}, ${orderData.address.state}, ${orderData.address.zip} `, 150, customerInformationTop + 30)
        .moveDown();
}
...
function generateInvoiceTable(doc, total, discount, orderData) {
    ...
    const discountPosition = invoiceTableTop + (orderData.products.length + 2) * 30;
